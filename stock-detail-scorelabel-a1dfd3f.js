...
function beginnerGuide(f = {}, rows = []) {
  ...
  const simpleWords = [
    `점수: ${Number.isFinite(score) ? `${score}점` : '대기'}입니다. 이 점수는 “매수강도”가 아니라 분석 신호의 강도라서, 판단 신뢰도·시장상태·수급과 따로 봅니다.`,
    `판단 신뢰도: ${survival.confidenceLevel || '-'} · ${survival.confidenceScore ?? '-'}점입니다. 기준은 High 75+, Medium 60~74, Low 45~59, Uncertain 45 미만입니다. 신뢰도가 낮으면 점수가 높아도 관망할 수 있습니다.`,
...
function renderDecisionBars(g = {}) {
  ...
  return `<div class="decision-visuals">
    ...
    ${bar('행동 판단 신뢰도', confidence, '실제 행동 비중에 더 중요')}
...
function renderThemeRegime(f = {}) {
  const t = f.themeRegime || f.expertAnalysis?.survival?.themeRegime || null;
  if (!t) return '';
  const policy = t.policy || {};
  const watch = Array.isArray(t.watchSignals) ? t.watchSignals : [];
  const risks = Array.isArray(t.riskRules) ? t.riskRules : [];
  return `<section class="card stock-detail-section theme-regime-card">
    <h2>테마 Regime</h2>
    <div class="theme-regime-head">
      <strong>${esc(t.name || 'Theme Regime')}</strong>
      <b>${esc(t.label || t.state || '-')} · 강도 ${esc(t.strengthScore ?? '-')}</b>
      <span>리스크 ${esc(t.riskLevel || '-')}</span>
    </div>
    <p>${esc(t.plain || '')}</p>
    <div class="scenario-grid">
      <article><h3>이 종목 적용</h3>${list([policy.label || '-', policy.plain || t.plain || '-', policy.pbrPenalty ? `PBR 해석: ${policy.pbrPenalty}` : ''])}</article>
      <article><h3>주기 점검 신호</h3>${list(watch.length ? watch : ['주도 테마/수급/밸류 프리미엄 점검 대기'])}</article>
      <article><h3>약화 조건</h3>${list(risks.length ? risks : ['테마 약화 조건 수집 대기'])}</article>
    </div>
  </section>`;
}
...
function renderSurvivalGuide(f = {}) {
  ...
  const tenSteps = [
    ...
    ['3. 행동 이유', s.confidenceLevel || '확인 필요', `점수와 행동을 분리합니다. 판단 신뢰도 ${s.confidenceScore ?? '-'}점 · 데이터 품질 ${s.dataQualityScore ?? '-'}점`],
  ...
  return `<section id="tab-survival" class="card stock-detail-section survival-guide"><h2>전문가용 10단계 판단</h2>
    <div class="stock-detail-metrics">
      ...
      ${metric('판단 신뢰도', `${s.confidenceLevel || '-'} · ${s.confidenceScore ?? '-'}점`, `High 75+ · Medium 60~74 · Low 45~59 · Uncertain 45 미만 · 데이터 품질 ${s.dataQualityScore ?? '-'}점`)}
...
function renderDetailTabs(tabs = []) {
  return `<nav class="stock-detail-tabs tabbed" aria-label="종목 상세 탭">
    ${tabs.map((tab, idx) => `<button type="button" class="stock-tab-btn ${idx === 0 ? 'active' : ''}" data-tab="${esc(tab.id)}">
      <strong>${esc(tab.label)}</strong><span>${esc(tab.summary || '')}</span>
    </button>`).join('')}
  </nav>`;
}

function renderTabbedSections(tabs = []) {
  return `<div class="stock-tab-panels">
    ${tabs.map((tab, idx) => `<section class="stock-tab-panel ${idx === 0 ? 'active' : ''}" data-panel="${esc(tab.id)}">
      <div class="stock-tab-panel-head"><h2>${esc(tab.label)}</h2><p>${esc(tab.summary || '')}</p></div>
      ${tab.html}
    </section>`).join('')}
  </div>`;
}
...
function bindStockTabs() {
  const buttons = Array.from(document.querySelectorAll('.stock-tab-btn'));
  ...
  if (initial && buttons.some(b => b.dataset.tab === initial)) activate(initial);
}

function confidenceSummary(f = {}) {
  const s = f.expertAnalysis?.survival || {};
  return `${s.actionState || '관망'} · 신뢰도 ${s.confidenceLevel || '-'} ${s.confidenceScore ?? '-'}점`;
}

function themeSummary(f = {}) {
  const t = f.themeRegime || f.expertAnalysis?.survival?.themeRegime || null;
  return t ? `${t.name || '테마'} ${t.label || t.state || '-'} · 강도 ${t.strengthScore ?? '-'}` : '테마 점검 대기';
}
  ...
  const tabs = [
    {
      id: 'summary',
      label: '행동 요약',
      summary: confidenceSummary(f),
      html: `${renderBeginnerGuide(f, ohlcv?.rows || [])}${renderSurvivalGuide(f)}`,
    },
    {
      id: 'theme',
      label: '테마/시장',
    ...
    },
    {
      id: 'price',
      label: '가격/타이밍',
      summary: `현재 ${money(tech.currentPrice)} · 전고점 ${pct(tech.distanceToHigh20dPct)}`,
      html: `${renderRiseTiming(f, ohlcv?.rows || [])}${renderRealtimeCandle(f)}<section id="tab-chart" class="card stock-detail-section"><h2>가격·거래량 구조 분석</h2>${renderOhlcv(ohlcv?.rows || [], tech, f.intradayCandle)}</section>`,
    },
    {
      id: 'valuation',
      label: '밸류/동종업계',
      summary: `PER ${f.per ?? '-'} · PBR ${f.pbr ?? '-'} · ROE ${f.roe ?? '-'}`,
      html: `<section id="tab-valuation" class="card stock-detail-section"><h2>ROE · PBR · PER 종합 밸류 분석</h2>${renderValuation(f)}${list(f.report)}</section>${renderPeerGrowthMargin(f)}`,
    },
    {
      id: 'flow',
      label: '수급/뉴스',
      summary: '실적·수급·공매도·뉴스',
      html: `${renderKisSummary(f)}${renderKisTables(f)}`,
    },
    {
      id: 'scenario',
      label: '시나리오/자료',
      summary: '조건·리스크·추가자료',
      html: `<section id="tab-scenario" class="card stock-detail-section"><h2>전문가형 시나리오 분석</h2>${scenarioTable(f)}</section><section class="card stock-detail-section"><h2>핵심 포인트</h2>${list(a.keyPoints)}</section><section class="card stock-detail-section"><h2>추가로 넣으면 좋은 자료</h2>${list(a.additionalDataNeeded, '현재 추가로 필요한 자료 없음')}<p class="stock-modal-note">${esc(a.disclaimer || '자동 분석은 투자 판단 보조자료입니다.')}</p></section>${contextBlock(contexts)}`,
    },
  ];
  root.innerHTML = `
    ...
    <section class="stock-detail-metrics top-summary-metrics">
      ...
      ${metric('판단 신뢰도', `${f.expertAnalysis?.survival?.confidenceLevel || '-'} · ${f.expertAnalysis?.survival?.confidenceScore ?? '-'}점`, 'High 75+ · Medium 60~74 · Low 45~59')}
      ${metric('테마 Regime', themeSummary(f), '주도 테마는 주기 점검')}
      ${metric('PER/PBR/ROE', `${f.per ?? '-'} / ${f.pbr ?? '-'} / ${f.roe ?? '-'}`, f.badge || '')}
    </section>
    ${renderDetailTabs(tabs)}
    ${renderTabbedSections(tabs)}
  `;
  bindStockTabs();

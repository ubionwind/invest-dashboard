...
function collectActionMatrix(data = {}) {
  ...
  const rowKey = (strategy, code, source = '') => {
    const normalized = String(code).padStart(6, '0');
    return source === 'universe' ? `universe::${normalized}` : `${strategy || '전략 미지정'}::${normalized}`;
  };
  (data.sessions || []).forEach(s => {
    ((s.portfolio || {}).positions || []).forEach(p => {
      if (p?.code && Number(p.qty || 0) > 0) {
        const code = String(p.code).padStart(6, '0');
        holdingCodes.add(code);
        holdingKeys.add(rowKey(s.name, code, 'holding'));
      }
    });
  });
  const add = (item, strategy, source) => {
    ...
    const key = rowKey(strategy, code, source);
    ...
    const isHolding = holdingKeys.has(rowKey(strategy, code, 'holding')) || (source === 'holding' && Number(item.qty || 0) > 0) || String(item.status || '').includes('보유중');
    const row = {
      key,
      code,
  ...
  (data.stockUniverse || []).forEach(x => {
    const code = String(x.code || '').padStart(6, '0');
    if (!holdingCodes.has(code)) add(x, x.strategy || '전체 분석', 'universe');
  });
  return Array.from(byKey.values()).sort((a, b) => a.signal.priority - b.signal.priority || (b.holdingState === '보유중') - (a.holdingState === '보유중') || String(a.strategy || '').localeCompare(String(b.strategy || ''), 'ko') || Number(b.score || 0) - Number(a.score || 0));
}

function renderActionMatrix(data = {}, strategyFilter = null) {
  const allRows = collectActionMatrix(data);
  const rows = strategyFilter ? allRows.filter(r => r.strategy === strategyFilter) : allRows;
  const groups = [
  ...
  const scorePlain = r => {
    ...
    if (!Number.isFinite(score)) return '전략 행동점수 산정 데이터가 부족해 원자료 확인이 먼저입니다.';
    if (score >= 85) return `${r.strategy || '해당 전략'} 기준 행동 신호가 매우 강합니다. 종목 상세의 종합점수와는 별도입니다.`;
    if (score >= 70) return `${r.strategy || '해당 전략'} 기준 우호 신호가 강한 편입니다. 조건 확인 후 우선 검토합니다.`;
    if (score >= 55) return `${r.strategy || '해당 전략'} 기준 가능성은 있지만 가격·수급·리스크 확인이 필요합니다.`;
    if (score >= 40) return `${r.strategy || '해당 전략'} 기준 행동 우선순위는 낮고 관찰·대기 성격이 강합니다.`;
    return `${r.strategy || '해당 전략'} 기준 신호가 약하거나 리스크 대비 근거가 부족합니다.`;
  };
  ...
  const conditionText = (r = {}, kind = 'new') => {
    const f = r.item?.fundamentals || r.fundamentals || {};
    const c = r.finalIntegratedDecision?.conditions || {};
    const a = f.expertAnalysis || {};
    const survival = a.survival || {};
    const tech = f.technicalStructure || {};
    const triggers = Array.isArray(a.upsideTriggers) ? a.upsideTriggers : [];
    const invalid = Array.isArray(survival.invalidationRules) ? survival.invalidationRules : [];
    if (kind === 'new') {
      const parts = [c.newBuy, triggers[0], tech.breakoutPrice ? `돌파 기준 ${money(tech.breakoutPrice)} 확인` : '', tech.supportLine?.from ? `지지선 ${money(tech.supportLine.from)} 이탈 없음` : ''].filter(Boolean);
      return parts.length ? parts.slice(0, 2).join(' / ') : '가격 돌파, 거래량 증가, 수급 개선 중 최소 1개 이상 확인';
    }
    if (kind === 'hold') {
      const parts = [c.hold, invalid[0], tech.supportLine?.from ? `${money(tech.supportLine.from)} 지지 유지` : ''].filter(Boolean);
      return parts.length ? parts.slice(0, 2).join(' / ') : '주요 지지선 유지와 수급 악화 없음';
    }
    if (kind === 'exit') {
      const parts = [c.exit, c.trailingStop, c.partialTakeProfit, invalid[0]].filter(Boolean);
      return parts.length ? parts.slice(0, 2).join(' / ') : '지지선 이탈, 수급 급악화, 모멘텀 소멸 여부 확인';
    }
    return '';
  };
  const decisionPlain = (kind, state = {}, r = {}) => {
    ...
    if (kind === 'new') {
      const cond = conditionText(r, 'new');
      if (state.allowed) return `신규 진입 후보입니다. 실행 전 확인 조건: ${cond}`;
      return `지금은 새로 사기보다 기존 보유 판단을 우선합니다. 제한 해제 조건: ${cond}`;
    }
    if (kind === 'hold') {
      const cond = conditionText(r, 'hold');
      if (state.allowed) return `이미 보유 중이면 유지 우선입니다. 계속 들고 갈 조건: ${cond}`;
      return `보유 지속 근거가 약합니다. 먼저 확인할 조건: ${cond}`;
    }
    if (kind === 'exit') {
      const cond = conditionText(r, 'exit');
      if (state.stopLoss) return `손실 확대 방어가 우선입니다. 청산 확인 조건: ${cond}`;
      if (state.partialTakeProfitAllowed) return `전량 매도보다 일부 익절/트레일링 검토입니다. 수익 보호 조건: ${cond}`;
      if (state.momentumCheck) return `매도 신호가 아니라 보유 근거 점검입니다. 점검 조건: ${cond}`;
      return `강한 청산 신호는 아닙니다. 그래도 확인할 조건: ${cond}`;
    }
    if (kind === 'exec') {
  ...
  const actionReadout = r => {
    ...
    return `<dl class="action-readout">
      ${row('전략 행동점수', r.score === null || r.score === undefined || r.score === '' ? '-' : `${Number(r.score).toFixed(1)}점`, scorePlain(r), Number(r.score) >= 85 ? 'score-hot' : Number(r.score) >= 70 ? 'score-strong' : Number(r.score) >= 55 ? 'score-mid' : 'score-low')}
  ...
  const cell = r => `<article class="matrix-stock ${r.signal.tone} ${r.holdingState === '보유중' ? 'holding' : 'watching'}" title="${escapeHtml(r.reason)}">
    ...
    <details class="action-detail">
      ...
      <ul class="action-detail-meta">
        <li><b>전략점수</b>${escapeHtml(r.score ?? '-')}<small>행동 현황판은 전략/보유 상태 기준 점수입니다. 종목 상세의 종합점수와 다를 수 있습니다.</small></li>
  ...
  return `<section class="card action-matrix-card">
    <div class="card-head">
      <div><h2>분석 종목 행동 현황판</h2><p class="muted">${strategyFilter ? `${escapeHtml(strategyFilter)} 전략 기준으로 필터링 중입니다.` : '전체 전략 기준입니다.'} 행동 분류 탭을 눌러 종목을 나눠 봅니다.</p></div>
      <span class="badge">${strategyFilter ? `${escapeHtml(strategyFilter)} · ` : ''}${rows.length}종목 · 보유 ${holdingCount}</span>
    </div>
...
function activeStrategyName(id) {
  const tab = dashboardStrategyTabs.find(t => t.id === id);
  return tab?.strategy || null;
}
...
function setActive(id, shouldScroll = true) {
  ...
  refreshActionMatrixForPanel(id);
...
function render(data) {
  ...
  refreshActionMatrixForPanel('panel-overview');

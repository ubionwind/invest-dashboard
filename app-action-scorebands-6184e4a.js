...
function renderActionMatrix(data = {}) {
  ...
  const cell = r => `<article class="matrix-stock ${r.signal.tone} ${r.holdingState === '보유중' ? 'holding' : 'watching'}" title="${escapeHtml(r.reason)}">
    ...
    <div class="action-brief">
      ...
      <dl>
        <div><dt>점수</dt><dd>${escapeHtml(r.score ?? '-')}점</dd></div>
  ...
  const scoreBands = [
    { key: 'high', label: '100~70점 구간', min: 70, max: 100, desc: '분석 신호가 강한 구간입니다. 다만 이 점수는 자동 매수/매도 지시가 아니라, 해당 행동 탭 안에서 우선 검토할 강도를 뜻합니다.' },
    { key: 'mid', label: '69~50점 구간', min: 50, max: 69.999, desc: '조건 확인이 필요한 중간 구간입니다. 가격·수급·보유상태가 맞으면 검토하고, 아니면 대기합니다.' },
    { key: 'low', label: '49점 이하 구간', min: -Infinity, max: 49.999, desc: '신호가 약하거나 데이터 확신이 부족한 구간입니다. 실행보다 관찰·보류·재검토 의미가 큽니다.' },
    { key: 'unknown', label: '점수 없음/확인 필요', min: null, max: null, desc: '점수 산정 데이터가 부족한 종목입니다. 상세 조건과 원자료 확인이 먼저입니다.' },
  ];
  const scoreValue = r => {

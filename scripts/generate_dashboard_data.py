...
AI_MEMORY_THEME = {
    ...
    'state': 'MAINTAINED',
    'label': '유지·변동성/과열 재진입 경계',
    'strengthScore': 78,
    'riskLevel': 'HIGH',
    'updatedAt': None,
    'plain': 'HBM 공급 제약, DRAM 강세와 AI CAPEX는 AI 메모리 프리미엄을 계속 지지합니다. 다만 8월 초 국내 대형 반도체의 급락 뒤 반등은 추세 재개 확인이 아니라 고변동성 국면을 뜻합니다. 프리미엄은 유지하되 신규 가중은 SOX·Nvidia·Micron·TSMC 상대강도와 국내 외국인 수급이 함께 안정될 때만 허용합니다.',
    ...
    'riskRules': ['AI CAPEX 둔화, HBM 판가 하락, DRAM 가격 피크아웃, SOX 약세가 겹치면 프리미엄을 약화/종료 검토로 낮춥니다.', 'SOX/엔비디아/마이크론/TSMC 조정과 국내 외국인 매도가 동시에 이어지면 신규 진입 가중치를 낮춥니다.', '국내 반도체가 급락 뒤 단기간 반등한 경우에는 반등률만으로 가중을 늘리지 않고, 20일 상대강도와 외국인 수급의 동시 개선을 확인합니다.', '국내 반도체 비중이 펀드 한도나 지수 집중도 때문에 강제 조절되는 구간은 실적 논리와 별개로 변동성 리스크로 봅니다.', 'AI 인프라 투자가 부채 조달 의존을 키우거나 고객 CAPEX 재검토로 번지면 HBM 수요의 지속성을 재점검합니다.', '삼성전자는 HBM4 인증 이후에도 실제 물량·수율·고객 점유율 확인 전까지 하이닉스보다 프리미엄을 할인합니다.'],
    ...
    'tacticalReview': {
        'asOf': '2026-08-09',
        'state': 'VOLATILITY_REBOUND',
        'plain': '8월 6일 국내 마감 기준 삼성전자·SK하이닉스는 20일 -16.9%/-28.0% 후 5일 +11.4%/+13.1%를 기록했습니다. 구조적 수요는 유지되지만, 이 반등만으로 추세 복귀나 가중 확대를 확정하지 않습니다.',
        'entryGate': ['SOX·Nvidia·Micron·TSMC 상대강도 회복', '국내 외국인 순매도 완화 또는 순매수 전환', 'DRAM/HBM 가격 강세 및 고객 CAPEX 유지'],
        'newLeadershipEvidence': '전력 인프라(LS ELECTRIC +34.7%, HD현대일렉트릭 +30.5%), HBM 장비(+19.6%), AI 네트워킹/고속기판(+40.4%)의 5일 강세를 확인했습니다. 단, 각 업종의 수주·마진 검증 전까지는 CANDIDATE로 유지합니다.'
    },
    'nextLeadershipCandidates': [
        {'id': 'power-infra-ai-grid', 'name': '전력기기/AI 전력 인프라', 'state': 'CANDIDATE', 'score': 88, 'plain': 'AI 데이터센터 증설이 전력망·변압기·전력기기 발주로 이어지는 구간이라 반도체 변동성 확대 시 가장 먼저 비교할 대체 주도 업종 후보입니다. 단기 상대강도는 확인됐지만 고밸류이므로 수주잔고·마진이 확인될 때만 주도 업종으로 승격합니다.', 'watchSignals': ['HD현대일렉트릭/LS ELECTRIC/효성중공업 상대강도', '미국 전력망·데이터센터 발주', '초고압 변압기 가격', '수주잔고와 마진', '고PBR 정당화 여부']},
    ...
    'codePolicy': {
        ...
        '005930': {'premium': 'PARTIAL', 'label': 'AI 수혜 일부 반영·HBM4 물량 검증 필요', 'pbrPenalty': 'partial', 'plain': '삼성전자는 HBM4 인증 기대와 범용 DRAM/NAND 회복을 반영하되 실제 물량·수율·고객 점유율 확인 전까지 하이닉스보다 할인합니다.'},
    },
}
